...
import { compilationError, logError } from "./error.js";
import { getFiles, writeFile, readFile, isDirectory, isFile, deleteAll } from "./file-system.js";
;
export async function compileMacros(opts) {
...
function compileFile(file, target, macrosHeader) {
    let cursor = 0;
    let compiledSrc = "";
    const copy = (until) => {
        ...
        switch (insideBlock) {
            ...
            default:
                if ("\"'`".includes(src[i])) {
                    ...
                    skipLine();
                    const header = [start + 3, headerEnd];
                    macros.push({
                        replace: [],
                        header,
                        body: [start, i],
