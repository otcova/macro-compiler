...
import { compileMacros } from "./index.js";
const args = process.argv.slice(2);
function main() {
    const options = { srcDir: args.shift() ?? "", clean: true };
    ...
    if (!flags)
        return;
    if (flags["--target"].length % 2 != 0)
        flags.target.length.push("");
    ...
    if (rootDir.length > 0) {
        if (rootDir.length == 2)
            logError(`Invalid argument '${rootDir[0]}'`);
        else
            logError(`Invalid arguments '${rootDir.join(" ")}'`);
        return;
    }
    ...
    if (cleanArray.length > 0) {
        if (rootDir.length == 2)
            logError(`Invalid argument '${cleanArray}'`);
        else
            logError(`Invalid arguments '${cleanArray.join(" ")}'`);
        return;
    }
    if (clean) {
        if (["false", "f", "0"].includes(clean))
            options.clean = false;
...
function parseFlags(validFlags) {
    const flags = {};
...
${style.bold}Example:${style.reset}  macro-compiler src -t NodeJs nodejs/src Browser browser/src
...
${style.bold}Flags:${style.reset}
  ...
  ${style.blue}--rootDir, -r${style.reset}  Specify the root folder
    ...
    ${style.blue}--clean, -c${style.reset}  Delete the targets direcories before compilation
